...
export function companyRoutes(
  ...
  routeOpts?: { anyUserCanCreateCompany?: boolean },
) {
  const router = Router();
  ...
  router.post("/", validate(createCompanySchema), async (req, res) => {
    assertBoard(req);
    const allowSessionUserCreate = Boolean(routeOpts?.anyUserCanCreateCompany && req.actor.source === "session");
    if (!(req.actor.source === "local_implicit" || req.actor.isInstanceAdmin || allowSessionUserCreate)) {
