...
export interface Config {
  ...
  /** When true, any board user with a browser session may POST /companies (not only instance admins). */
  anyUserCanCreateCompany: boolean;
...
export function loadConfig(): Config {
  ...
  const companyDeletionEnabled =
    companyDeletionEnvRaw !== undefined
      ...
      : deploymentMode === "local_trusted";
  const anyUserCanCreateCompanyEnv = process.env.PAPERCLIP_ANY_USER_CAN_CREATE_COMPANY;
  const anyUserCanCreateCompany =
    anyUserCanCreateCompanyEnv !== undefined ? anyUserCanCreateCompanyEnv === "true" : false;
